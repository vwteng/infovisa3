...
function drawVis(data) {
    ...
    }).style("opacity", 1).on("mouseover", function(d) {
        tooltip.transition().duration(200).style("opacity", 1);
        tooltip.html("<b>" + d.country.toUpperCase() + "<br>" +
            "CDR: </b>" + d.death + "<br>" + "<b>Region: </b>" +
            d.region + "<br>" + "<b> Year: </b>" + d.year).style(
            "left", "725px").style("top", "100px").style(
            "text-align", "center").style("background-color",
            "white").style("z-index", "1")
        svg.append("path").data(dataset).attr("class", "line").attr(
            "d", line)
...
svg.append("g").attr("class", "axis").call(yAxis).append("text").attr(
    ...
    "text-anchor", "end").text("Crude Death Rate (Per 1,000 People)");
